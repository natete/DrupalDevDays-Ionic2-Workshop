...
Finally we need to add a **allow-navigation** rule to our **config.xml**. This file is a Cordova configuration file where
we can define many aspects of our application. More on config.xml [here](https://cordova.apache.org/docs/en/6.x/config_ref/index.htmlhttps://cordova.apache.org/docs/en/6.x/config_ref/index.html)
```xml
...
using device with the livereload feature (the -l flag we added), so we need to run the application without it:
```bash
ionic run android
```
